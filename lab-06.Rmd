...
diamonds <- diamonds %>% filter(length_mm != 0) %>% filter(width_mm != 0) %>% filter(depth_mm != 0)
...
diamonds %>% summarise(avg_table_mm = mean(table_mm, na.rm = T))




# table_mm arithmetic mean within each clarity category
diamonds %>% group_by(clarity) %>% summarise(avg_table_mm = mean(table_mm, na.rm = T))
